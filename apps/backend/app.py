...
# UPDATED  FUNCTION  –  now accepts `namespace` for user segregation
def upload_to_pinecone(chunks: List[Dict], namespace: str = "") -> int:
    """
    Upload chunks to Pinecone.

    Parameters
    chunks      : List of chunk dicts (output of process_file)
    namespace   : Pinecone namespace (use userId for multi-tenant)
                  If empty, defaults to config.namespace_dense.
    """
    if not chunks:
        return 0
    ...
    for i in range(0, len(valid_chunks), config.batch_size):
        ...
        try:
            response = index.upsert(
                vectors=batch,
                namespace=namespace or config.namespace_dense
            )
            uploaded += len(batch)
...
def process_directory_parallel(directory: str):
    ...
    total_uploaded = upload_to_pinecone(all_chunks)   # default namespace
